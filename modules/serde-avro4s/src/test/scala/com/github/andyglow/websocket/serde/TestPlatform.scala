...
import com.github.andyglow.websocket.{Platform, ServerAddress}
...
class TestPlatform extends Platform {
  ...
  override val implicits: MessageAdapter.Implicits with Implicits = new MessageAdapter.Implicits with Implicits {
    override implicit val StringMessageAdapter: MessageAdapter.Aux[String, Msg.Text] = MessageAdapter(Msg.Text.apply, _.value)
    override implicit val CharArrayMessageAdapter: MessageAdapter.Aux[Array[Char], Msg.Text] = MessageAdapter(x => Msg.Text(new String(x)), _.value.toCharArray)
    override implicit val CharBufferMessageAdapter: MessageAdapter.Aux[CharBuffer, Msg.Text] = MessageAdapter(x => Msg.Text(x.asString), x => CharBuffer.wrap(x.value))
    override implicit val ByteArrayMessageAdapter: MessageAdapter.Aux[Array[Byte], Msg.Binary] = MessageAdapter(Msg.Binary.apply, _.value)
    override implicit val ByteBufferMessageAdapter: MessageAdapter.Aux[ByteBuffer, Msg.Binary] = MessageAdapter(x => Msg.Binary(x.asByteArray), x => ByteBuffer.wrap(x.value))
  }
  ...
  override def newClient(address: ServerAddress, options: Options = defaultOptions): WebsocketClient = new WebsocketClient {
    override def open(handler: WebsocketHandler): Websocket = new Websocket {
      override protected def send(x: Msg): Unit = if (handler.onMessage.isDefinedAt(x)) handler.onMessage(x) else handler.onUnhandledMessage(x)
