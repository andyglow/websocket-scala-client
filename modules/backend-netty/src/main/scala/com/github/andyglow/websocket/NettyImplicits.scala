...
trait NettyImplicits { this: NettyPlatform =>
  ...
  class NettyImplicits extends MessageAdapter.Implicits {
    ...
    private class TextAdapter[T <: CharSequence](ff: Text => T) extends MessageAdapter[T] {
      ...
      override def toMessage(msg: T): F   = new TextWebSocketFrame(Unpooled.copiedBuffer(msg, UTF_8))
    ...
    implicit val CharArrayMessageAdapter: MessageAdapter.Aux[Array[Char], Text] = new MessageAdapter[Array[Char]] {
      override type F = Text
      override def toMessage(msg: Array[Char]): F = new TextWebSocketFrame(Unpooled.copiedBuffer(msg, UTF_8))
      override def fromMessage(msg: TextWebSocketFrame): Array[Char] = msg.text().toCharArray
    }

    implicit val ByteBufTB: MessageAdapter.Aux[ByteBuf, Binary] = new MessageAdapter[ByteBuf] {
      override type F = Binary
      override def toMessage(msg: ByteBuf): F                      = new BinaryWebSocketFrame(msg)
      override def fromMessage(msg: BinaryWebSocketFrame): ByteBuf = msg.content()
    }

    implicit val ByteBufferMessageAdapter: MessageAdapter.Aux[ByteBuffer, Binary] = new MessageAdapter[ByteBuffer] {
      override type F = Binary
      override def toMessage(msg: ByteBuffer): F = new BinaryWebSocketFrame(Unpooled.wrappedBuffer(msg))
      override def fromMessage(msg: BinaryWebSocketFrame): ByteBuffer = msg.content().nioBuffer()
    }

    implicit val ByteArrayMessageAdapter: MessageAdapter.Aux[Array[Byte], Binary] = new MessageAdapter[Array[Byte]] {
      override type F = Binary
      override def toMessage(msg: Array[Byte]): F = new BinaryWebSocketFrame(Unpooled.wrappedBuffer(msg))
      override def fromMessage(msg: BinaryWebSocketFrame): Array[Byte] = {
        val c = msg.content()
